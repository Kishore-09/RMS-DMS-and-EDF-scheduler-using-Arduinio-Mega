...
static void prvCreateAllTasks( void );
	...
	static BaseType_t prvFindEmptyElementIndexTCB( void )
	{
		...
		for( uxIndex = 0; uxIndex < schedMAX_NUMBER_OF_PERIODIC_TASKS; uxIndex++)
		{
			if (xTCBArray[ uxIndex ].xInUse == pdFALSE) /*check this*/
			{
				return uxIndex; 
			}
		}
...
static void prvPeriodicTaskCode( void *pvParameters)
{
    ...
    */
	...
	{	
		...
		Serial.println("this task is running");
		pxThisTask->pvTaskCode(pvParameters);
		Serial.println(pxThisTask->pcName);
		Serial.println("this task is terminated");
		TickType_t currentTime = xTaskGetTickCount();
        pxThisTask->xAbsoluteDeadline =  currentTime + pxThisTask->xPeriod + pxThisTask->xRelativeDeadline;
...
static void prvSetFixedPriorities( void )
{
	...
	for( xIter = 0; xIter < xTaskCounter; xIter++ )
	{
		...
		for( xIndex = 0; xIndex < xTaskCounter; xIndex++ )
		{
			pxTCB = &xTCBArray[ xIndex ];
			#if( schedSCHEDULING_POLICY == schedSCHEDULING_POLICY_DMS )
				if(pxTCB->xAbsoluteDeadline < xShortest && pxTCB->xAbsoluteDeadline > xPreviousShortest && pxTCB->xInUse == pdTRUE)
				{
		...
		if(pxShortestTaskPointer != NULL)
		{
			pxShortestTaskPointer->uxPriority = configMAX_PRIORITIES-iPriority;
			iPriority++;
	...
	static void prvPeriodicTaskRecreate( SchedTCB_t *pxTCB )
	{
		...
		if( pdPASS == xReturnValue )
		{
			...
			pxTCB->xAbsoluteDeadline = xTaskGetTickCount + pxTCB->xRelativeDeadline;
	...
	static void prvDeadlineMissedHook( SchedTCB_t *pxTCB, TickType_t xTickCount )
	{
		...
		pxTCB->xLastWakeTime = xTickCount - (xTickCount%pxTCB->xPeriod) + pxTCB->xPeriod;
	}
	...
	{ 
		...
		if( xTickCount > pxTCB->xAbsoluteDeadline)
		{
	...
	static void prvSchedulerFunction( void *pvParameters )
	{
		...
		{ 
				...
				/* Iterate through all tasks, using pxTCB to reference each task's control block, and call prvSchedulerCheckTimingError(xTickCount, pxTCB) and prvExecTimeExceedHook(xTickCount, pxTCB) for each task to detect and handle any deadline misses or execution time overruns based on the current tick count. */				
